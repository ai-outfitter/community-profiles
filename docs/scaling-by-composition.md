...
- `prompts/practice.pull-request-approval.md` — the authority boundary for an
  ...
  blocking finding receives `REQUEST_CHANGES`. It never grants merge or
...
Resident review carriers compose the separate approval practice. They may
approve another author's clean current head, but they never review their own
work and never merge.
